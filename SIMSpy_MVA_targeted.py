...
itmfiles=["E:/Data/TOF_SIMS/UCL/110825_Sheaths/20250812_Cable bacteria/N_Spot I_A1.itm"] #put itmfiles here
...
"E:/Data/TOF_SIMS/UCL/110825_Sheaths/20250812_Cable bacteria/P_Spot II_B1.itm", #negative
...
itmfiles=["E:/Data/TOF_SIMS/UCL/110825_Sheaths/20250812_Cable bacteria/P_Spot II_B1.itm"]
# itmfiles=["E:/Data/TOF_SIMS/UCL/110825_Sheaths/20250812_Cable bacteria/N_Spot I_A1.itm"] #put itmfiles here
# itmfiles=["E:/Data/TOF_SIMS/UCL/110825_Sheaths/20250812_Cable bacteria/N_Spot VI_A1.itm"]

# itmfiles=["E:/Data/TOF_SIMS/UCL/110825_Sheaths/20250812_Cable bacteria/P_Spot VI_B1.itm"]
...
def Plot3D(cmat,name,denoise=False,surface_count=5):
    ...
    fig.write_html(fs+"_3D_"+name+".html")
...
for itmfile in itmfiles:
    ...
    tards=ds.loc[ds.peak_bin.isin(ldf.peak_bin),["scan","x","y","peak_bin"]]
    
    #binning
    tards.scan=(tards.scan/Target_bin_scans).astype(int)
    tards[["x","y"]]=( tards[["x","y"]]/Target_bin_pixels).astype(int)
    scans,xpix,ypix=int(scans/Target_bin_scans)+1,int(xpix/Target_bin_pixels)+1,int(ypix/Target_bin_pixels)+1
    
    
    ccol,groups="peak_bin",ldf.Formula.values
    if Sum_by_Groups:   
        tards["Group"]=ldf.set_index("peak_bin").loc[tards.peak_bin,"Group"].values
        ccol="Group"
    ...
    if Target_dimension==2:
        ...
        if tile_plots:
            row_entries=6
            n_components=len(ldf)
            rows,cols=n_components//row_entries,row_entries
            ...
            for n,g in utards.groupby(ccol):
                ...
                if Sum_by_Groups: axes[i].title.set_text(n)
                else: axes[i].title.set_text(", ".join(ldf.loc[ldf.peak_bin==n,"Formula"].values.tolist()))
                ...
                if i>=(rows-1)*cols: axes[i].set_xlabel(r'x $\mu$m') #only add to last row
            i+=1
            fig.savefig(fs+"_2D_Targets.png",dpi=300)
            ...
            for n,g in utards.groupby(ccol):
                ...
                if Sum_by_Groups:
                    plt.title(n)
                    fig.savefig(fs+"_2D_"+n+".png",dpi=300)
                
                else:
                    t=ldf.loc[ldf.peak_bin==n,"Formula"].values.tolist()
                    plt.title(", ".join(t))
                    fig.savefig(fs+"_2D"+"_".join([i[:-1] for i in t])+".png",dpi=300)
  ...
  #%%
    ...
    if Target_dimension==3:
        ...
        for n,g in utards.groupby(ccol):
        #for n,vdf in utards.groupby(ccol):
            ...
            #do pairwise as a function at the end
    ...
    for i in np.arange(len(pairs)):
        for j in np.arange(len(pairs)):
            if i!=j:
                
                if Target_dimension==2:
                
                    fig,ax=plt.subplots()
                    sns.heatmap((pairs[i]-pairs[j]).T)
                    plt.title(groups[i]+" - "+groups[j])
                    
                if Target_dimension==3:
      
                    Plot3D(pairs[i]-pairs[j],
                           name=groups[i]+" - "+groups[j],
                           surface_count=5)
    ...
    ## untargeted addition of other peaks to clusters
